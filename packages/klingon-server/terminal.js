...
app.post('/terminals', (req, res) => {
    ...
    //     env: process.env
    // });
    const term = cp.spawn(process.platform === 'win32' ? 'cmd.exe' : 'bash', [], {
        name: 'xterm-color',
        cols: cols || 80,
        rows: rows || 24,
        cwd: '/Users/wassimchegham/Sandbox/oss/klingon-workspace', //process.env.PWD
...
app.ws('/terminals/:pid', (ws, req) => {
    ...
    ws.send(JSON.stringify({ stdout: logs[term.pid] }));

    term.stdout.setEncoding('utf8');
    term.stdout.on('data', (stdout) => {
        try {
    ...
    term.stderr.on('data', (stderr) => {
        try {
            ws.send(JSON.stringify({ stderr }));
        } catch (ex) {
            // The WebSocket is not open, ignore
        }
    });
